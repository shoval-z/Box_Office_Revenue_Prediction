...
def data_prep(train_df):
    ...
    # x_train = train_df[['popularity', 'runtime', 'video', 'vote_average', 'vote_count', 'is_original_language_en', 'has_homepage', 'update_budget', 'month', 'is_in_popular_production', 'is_country_us', 'has_collection', 'amount_of_genre', 'year', 'Fantasy', 'Action', 'Adventure', 'Comedy', 'Horror', 'Documentary', 'Mystery', 'Thriller', 'inflation_Budget']]
    x_train = train_df[['is_in_popular_producer','is_in_popular_executive_producer','is_in_popular_director','is_in_popular_actores','runtime', 'vote_average', 'vote_count', 'update_budget', 'is_in_popular_production', 'has_collection', 'amount_of_genre', 'year', 'Action', 'Documentary', 'Drama', 'Crime', 'History', 'Animation', 'budget_year_ratio']]
...
# print('start train')
# gp.fit(new_train_x, y_train)
#
# y_pred = [np.maximum(i,1e-10) for i in gp.predict(new_test_x)]
#
# RMSLE = mean_squared_log_error(y_test, y_pred, squared=True)
# print('GP RMSLE: ', RMSLE)
# print('total time: ', time.time()-S)

# SVR
# svm_reg = SVR(kernel="rbf")
# svm_reg.fit(new_train_x, y_train)
# y_pred = svm_reg.predict(new_test_x)
#
# RMSLE = np.sqrt(mean_squared_log_error(y_test, y_pred))
# RMSE = np.sqrt(mean_squared_error(y_test, y_pred))
# print('SVR RMSLE: ', RMSLE)
# print('SVR RMSE: ', RMSE)
# print('total time: ', time.time()-S)
...
#adaboost
# model = AdaBoostRegressor(random_state=40,learning_rate=0.001,n_estimators=100000)
# model.fit(new_train_x, y_train)
# y_pred = model.predict(new_test_x)
# y_pred = np.expm1(y_pred)
# RMSLE = np.sqrt(mean_squared_log_error(y_test, y_pred))
# RMSE = np.sqrt(mean_squared_error(y_test, y_pred))
# print('adaboost RMSLE: ', RMSLE)
