...
def data_prep(train_df, mode='train', year_bin=False):
    if mode == 'test':
        object_dict = pickle.load(open(os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])),'data/relevant_data.pkl'), "rb"))
    ...
    if mode == 'train':
        ...
        pickle.dump(object_dict, open('data/relevant_data.pkl', "wb"))
